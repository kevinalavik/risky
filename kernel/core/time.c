...
void time_init(const struct fdt *fdt) {
  ...
  if (fdt != 0 && fdt_find_path(fdt, "/cpus", &cpus) &&
      fdt_read_u32(&cpus, "timebase-frequency", &frequency) && frequency != 0) {
...
uint64_t time_now_ticks(void) { return arch_read_time() - g_boot_ticks; }

uint64_t time_frequency_hz(void) { return g_timebase_frequency; }
...
void time_get_uptime(struct time_uptime *uptime) {
  ...
  uptime->milliseconds =
      (uint32_t)((ticks % g_timebase_frequency) * 1000UL / g_timebase_frequency);
}
