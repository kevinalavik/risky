...
bool fdt_property_read_u32(const struct fdt_property *property, uint32_t *value)
{
	if (property == NULL || value == NULL ||
		property->length < sizeof(uint32_t)) {
		return false;
	}
...
bool fdt_property_read_u64(const struct fdt_property *property, uint64_t *value)
{
	if (property == NULL || value == NULL ||
		property->length < sizeof(uint64_t)) {
		return false;
	}

	const uint8_t *data = (const uint8_t *)property->data;
	*value = ((uint64_t)data[0] << 56) | ((uint64_t)data[1] << 48) |
				  ...
				  uint64_t *value)
{
	...
	return fdt_property_read_u64(&property, value);
}

static struct fdt g_fdt;
static bool g_fdt_valid = false;

bool fdt_global_init(const void *blob)
{
	g_fdt_valid = fdt_init(&g_fdt, blob);
	return g_fdt_valid;
}
